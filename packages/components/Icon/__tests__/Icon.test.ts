...
// 字符串名字能被解析的前提：图标已注册进 FontAwesome 的 library
library.add(faUser);

describe("图标渲染", () => {
  it("渲染图标组件中的根元素，并把icon Prop交给FontAwesome组件渲染成svg", () => {
  ...
  it("将color Prop应用在图标组件中的根元素的内联样式上", () => {
  ...
  it("type、color Prop不透传给FontAwesome组件", () => {
    const wrapper = mount(Icon, {
